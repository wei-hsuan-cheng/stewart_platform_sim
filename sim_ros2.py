...
class StewartTFBroadcaster(Node):
    def __init__(self):
        ...
        self.leg_pub = self.create_publisher(Float64MultiArray, '/stewart_platform/leg_lengths', 10)
        self.marker_pubs = []
        for i in range(10):
    ...
    def compute_leg_lengths(self, positions_b_bsj, positions_b_esj):
        """Calculate leg lengths based on spherical joint positions."""
        leg_lengths = np.zeros(6)
        for i in range(6):
            leg_lengths[i] = self.Norm(positions_b_esj[i] - positions_b_bsj[i])
        
        return leg_lengths
    
    def verify_leg_lengths(self):
        """Verify the leg lengths by TFs."""
        # Compute leg lengths again using LookUpTF
        leg_lengths = np.zeros(6)
        for i in range(6):
            ...
            leg_lengths[i] = self.Norm(pos_esj - pos_bsj)
    ...
    def get_target_pose(self):
        ...
        self.target_axis_ang_b_e = np.array([0.0, 1.0, 0.0, 
                                             (np.pi / 12.0) * np.sin(2 * np.pi * 0.1 * self.t_),
                                             ])
    ...
    def solve_ik(self):
        ...
        self.s = self.compute_leg_lengths(self.positions_b_bsj, self.positions_b_esj)
        print(f"\nIK solution (leg lengths) [m] = {self.s}")
        
    def publish_leg_lengths(self):
    ...
    def broadcast_tf(self):
        ...
        # # Publish spherical joints at end-effector {esj_i} (w.r.t. base)
        # for i in range(len(self.positions_b_esj)):
        #     # Publish spherical joints at end-effector {esj_i}
        #     self.publish_tf('base', f'b_esj_{i+1}', self.positions_b_esj[i], self.target_quat_b_e, now)

    def verify_ik(self):
        """Verify the IK solution by checking the leg lengths."""
        self.s_verify = self.verify_leg_lengths()
        self.length_error = self.s - self.s_verify
        
        # if self.Norm(self.length_error) > 10 ** (-5):
        #     print(f"\nIK error (leg lengths error) [m] = {self.length_error}")
        # else:
        #     print("\nIK solution verified successfully!")  
